...
`screenshots/`, plus `screen.png` and `thumbnail.png` at the repo root (kept
there since they're referenced by URL from obsidian-releases'
`community-css-themes.json` for Obsidian's in-app theme browser). Mobile
shots use Obsidian's mobile emulation on the desktop app at the iPhone 16
Pro's viewport and pixel ratio, so they are close to but not identical to the
real iOS app. Edit `test/vault/Kitchen Sink.md` to change what gets captured.
