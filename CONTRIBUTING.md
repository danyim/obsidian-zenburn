...
opens `test/vault` with the theme installed, and overwrites `screen.png`,
`settings.png` and the `mobile*.png` files. Mobile shots use Obsidian's
mobile emulation on the desktop app at 390x844, so they are close to but
not identical to the real iOS app. Edit `test/vault/Kitchen Sink.md` to
